...
## Your name:Amanda Gomez
## The names of anyone you worked with on this project:Kayla Williams
...
def get_user_tweets(user):
    if user in CACHE_DICTION:
        ...
        response = api.user_timeline(screen_name = user, count = 20)
        print(response)
...
for tw in umich_tweets:
    stuff = 'INSERT OR IGNORE INTO Tweets (tweet_id, tweet_text, user_posted, time_posted, retweets) VALUES (?,?,?,?,?)'
    tup1=tw['id_str'], tw['text'], tw['user']['id_str'],tw['user']['created_at'], tw['retweet_count']
    cur.execute(stuff, tup1)
    #creating values in table Users based on keys in string
    for to in tw['entities']['user_mentions']:
        t = api.get_user(to['screen_name'])
        tup= t['id_str'], t['screen_name'], t['favourites_count'],t['description']
        stuff = 'INSERT OR IGNORE INTO Users (user_id, screen_name, num_favs, description) VALUES (?,?,?,?)'
        cur.execute(stuff, tup)
...
s= cur.fetchall()
favorites=[x[0] for x in s]

# Make a query using an INNER JOIN to get a list of tuples with 2
# elements in each tuple: the user screenname and the text of the
# tweet. Save the resulting list of tuples in a variable called joined_data.
joined_data = True
cur.execute('SELECT screen_name, tweet_text FROM Users INNER JOIN Tweets ON Users.user_id= Tweets.user_posted')
...
joined_data2 = True
cur.execute('SELECT Users.screen_name, Tweets.tweet_text FROM Users JOIN Tweets ON Users.user_id= Tweets.user_posted ORDER BY retweets DESC')
